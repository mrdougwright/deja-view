...
function evaluate(expr, view) {
	...
	while(stack.length) {
		...
		if(call.apply_fn) {
			...
			var output = call.apply_fn.apply(view, args)
			result.push(output)
		}
			...
			for(var i = 0; i < sub_exprs.length; ++i) {
				...
				if(term.val) result.push(term.val) // Parsed value (num/str/bool)
					...
					if(typeof val === "function") {
						...
						stack.splice(stack.length - param_len, 0, {apply_fn: val, param_len: param_len})
					}
					...
					// func to its unparsed arguments)
						...
						stack.push({apply_fn: val._lazy, param_len: param_len})
					}
