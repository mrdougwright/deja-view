...
var evaluate = module.exports = function(expr, view, node, commentNode) {
	...
	// Apply a function
		...
		if(typeof val === "function") return val.apply(view, subExprs.slice(1).concat([node, commentNode]))
		else return val || ''
	}
	...
	// with this being recursive since it's rare
		...
		return evaluate(atom, view).apply(view, subExprs.slice(1), node, commentNode)
}
